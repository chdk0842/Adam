...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.formLogin()
			...
			.defaultSuccessUrl("/member/userMain") //로그인 성공시 이동할 페이지
			.usernameParameter("userId") //로그인시 사용할 파라메터 이름
		...
		http.authorizeRequests()
		    ...
		    .mvcMatchers("/", "/member/**", "/project/**",  "/images/**", "/favicon/**").permitAll() //모든 사용자가 로그인(인증) 없이 접근할 수 있도록 설정
